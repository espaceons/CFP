...
# importation des model Session et Seance
from gestion_formations.models import Session, Seance
...
class Instructor(models.Model):
    ...
    user = models.OneToOneField(
        CustomUser,
        # Si l'utilisateur est supprimé, le profil  formateur supprimé
        on_delete=models.CASCADE,
        primary_key=True,         # Utilise le user_id comme clé primaire pour simplifier
        # Permet d'accéder au profil formateur depuis l'utilisateur (user.instructor)
...
class Student(models.Model):
    ...
    user = models.OneToOneField(
        ...
        primary_key=True,
        # Permet d'accéder au profil étudiant depuis l'utilisateur (user.student)
...
class EnrollmentStatus(models.TextChoices):
    ...
    # Inscription approuvée par l'administration
    APPROVED = 'APPROVED', _('Approuvée')
    # Inscription rejetée par l'administration
    REJECTED = 'REJECTED', _('Rejetée')
    # Annulée par l'étudiant ou l'administration
...
class Enrollment(models.Model):  # innscrit
    ...
    student = models.ForeignKey(
        ...
        # Permet d'accéder aux inscriptions d'un étudiant exp : student.enrollments.all()
        related_name='enrollments',
        # Utilise le modèle Student pour représenter l'étudiant
        verbose_name=_("Étudiant")
    )
    session = models.ForeignKey(
        # En utilisant les chaînes de caractères pour les références entre ces deux applications, vous cassez la dépendance circulaire et Django pourra charger vos modèles correctement.
        'gestion_formations.Session',
        # Si la session est supprimée, ses inscriptions sont supprimées
        on_delete=models.CASCADE,
        # Permet d'accéder aux inscriptions d'une session exp : session.enrollments.all()
        related_name='enrollments',
        # Utilise le modèle Session pour représenter la session exp : session.nom_session
        verbose_name=_("Session")
    )
    date_inscription = models.DateField(auto_now_add=True, verbose_name=_(
        "Date d'inscription"))  # Date automatique de création de l'inscription
    ...
    date_statut = models.DateField(blank=True, null=True, verbose_name=_(
        "Date de changement de statut"))  # Date à laquelle le statut a été modifié
...
class Attendance(models.Model):  # presence specifique
    ...
    enrollment = models.ForeignKey(
        Enrollment,
        # Si l'inscription est supprimée, les enregistrements de présence associés le sont aussi
        on_delete=models.CASCADE,
        # Permet d'accéder aux présences depuis une inscription exp : Enrollment.attendances.all()
        related_name='attendances',
        verbose_name=_("Inscription")  # Inscription à la séance exp
    )
    seance = models.ForeignKey(
        'gestion_formations.Seance',
        # Si la séance est supprimée, ses enregistrements de présence le sont aussi
        on_delete=models.CASCADE,
    ...
    status = models.CharField(
        ...
        # Par défaut, on considère l'étudiant comme absent s'il n'est pas marqué
        default=AttendanceStatus.ABSENT,
        verbose_name=_("Statut de présence")
    )
    date_enregistrement = models.DateTimeField(auto_now_add=True, verbose_name=_(
        "Date d'enregistrement"))  # Quand la présence a été enregistrée
